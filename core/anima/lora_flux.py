...
# The original trainer module pulled in ``diffusers.AutoencoderKL`` and
# ``transformers.CLIPTextModel`` purely as type hints. ``from __future__ import
# annotations`` makes those hints strings, so we don't need diffusers installed
# for inference. Aliases below keep the unresolved names available for any
# runtime ``isinstance`` checks.
AutoencoderKL = Any  # type: ignore[assignment]
CLIPTextModel = Any  # type: ignore[assignment]
