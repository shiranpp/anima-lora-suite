...
class MiniTrainDIT(nn.Module):
    ...
    def prepare_embedded_sequence(
        ...
        if self.concat_padding_mask:
            # NEAREST resize of the (..., H, W) padding mask to the latent's spatial
            # size. Uses F.interpolate instead of torchvision.transforms.resize so the
            # preview has no torchvision dependency (which would need to version-match
            # the user's torch/CUDA build). torchvision's NEAREST resize calls the same
            # F.interpolate under the hood, so this is behaviour-identical.
            target_hw = tuple(x_B_C_T_H_W.shape[-2:])
            *lead, mh, mw = padding_mask.shape
            if (mh, mw) != target_hw:
                flat = padding_mask.reshape(-1, 1, mh, mw)
                flat = F.interpolate(flat, size=target_hw, mode="nearest")
                padding_mask = flat.reshape(*lead, *target_hw)
