...
def load_anima_dit(
    ...
    if disable_mmap:
        from .safetensors_utils import load_safetensors as load_safetensors_no_mmap
...
def load_anima_vae(vae_path: str, dtype: torch.dtype = torch.float32, device: str = "cpu"):
    ...
    """
    from .anima_models import ANIMA_VAE_MEAN, ANIMA_VAE_STD
    ...
    if os.path.isdir(qwen3_path):
        ...
        if not os.path.exists(config_dir):
            raise FileNotFoundError(
                ...
                "This ships with the repo under core/configs/qwen3_06b/ (config.json, "
                "tokenizer.json, etc.); your checkout appears to be missing it. Re-pull the "
                "repository, or restore the files from the Qwen3-0.6B HuggingFace repo."
            )
...
def load_t5_tokenizer(t5_tokenizer_path: Optional[str] = None):
    ...
    raise FileNotFoundError(
        ...
        "This ships with the repo under core/configs/t5_old/ (spiece.model and "
        "tokenizer.json); your checkout appears to be missing it. Re-pull the "
        "repository, or restore spiece.model from google/t5-v1_1-xxl (the fast "
        "tokenizer.json is derived from it)."
    )
