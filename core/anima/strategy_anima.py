...
# The trainer's ``train_util`` exposed a ``HIGH_VRAM`` flag and a
# ``clean_memory_on_device`` re-export. The editor only ever runs inference, so
# we always free VRAM after VAE encode — equivalent to the trainer's
# ``HIGH_VRAM=False`` default.
HIGH_VRAM = False
